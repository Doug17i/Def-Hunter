...
all_train_releases = {'activemq': 'activemq-5.0.0', 'camel': 'camel-1.4.0', 'derby': 'derby-10.2.1.6', 
                      ...
                      'mixture':['mixture-1', 'mixture-2'],
                      'activemq-ft': 'activemq-5.0.0-ft', 'camel-ft': 'camel-1.4.0-ft', 'derby-ft': 'derby-10.2.1.6-ft',
                      'groovy-ft': 'groovy-1_5_7-ft', 'hbase-ft': 'hbase-0.94.0-ft', 'hive-ft': 'hive-0.9.0-ft',
                      'jruby-ft': 'jruby-1.1-ft', 'lucene-ft': 'lucene-2.3.0-ft', 'wicket-ft': 'wicket-1.3.0-incubating-beta-1-ft',
                      }
...
word2vec_dir = '../../../output/Word2Vec_model/DeepLineDP'

def get_df(file_paths, is_baseline=False):
    dfs = []

    for file_path in file_paths:
        if is_baseline:
            df = pd.read_csv('../' + file_lvl_gt + file_path + ".csv")
        else:
            df = pd.read_csv(file_lvl_gt + file_path + ".csv")
...
def get_df2(rel, is_baseline=False):
...
def get_code3d_and_label(df, to_lowercase = True):
    '''
        input
...
def get_dataloader(code_vec, label_list,batch_size, max_sent_len):
    ...
    dl = DataLoader(tensor_dataset,shuffle=False,batch_size=batch_size,drop_last=True)
