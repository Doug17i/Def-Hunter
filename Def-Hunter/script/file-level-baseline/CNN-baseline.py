...
def train_model(dataset_name):
    ...
    train_df = get_df2(train_rel,is_baseline=True)
    
    valid_df = get_df2(valid_rel,is_baseline=True)
    ...
    train_dl = get_dataloader2(word2vec_model, train_code,train_label, padding_idx, batch_size)
    valid_dl = get_dataloader2(word2vec_model, valid_code,valid_label, padding_idx, batch_size)
...
def predict_defective_files_in_releases(dataset_name, target_epochs = 100):
    ...
    for rel in eval_rels:
        ...
        test_df = get_df2(rel, is_baseline=True)
...
for dataset_name11 in ["activemq", "camel", "derby", "groovy", "hbase", "hive", "jruby", "lucene", "wicket"]:
  proj_name = dataset_name11
if args.train:
